function Button({ children, disabled, type, onClick }) {
  ...
  return (
    <button
      disabled={disabled}
      onClick={onClick}
      className={`${additionalClass} flex text-lg items-center gap-1 px-3 py-2 font-semibold transition-all rounded-lg disabled:cursor-not-allowed disabled:bg-gray-500 disabled:text-gray-300`}
    >
